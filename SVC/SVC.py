...
pred2_svc = svc_model2.predict(X2_test)
print('Book2 SVC Accuracy:', accuracy_score(y2_test, pred2_svc))
print(classification_report(y2_test, pred2_svc))
...
svc_model3 = SVC(random_state=42)
svc_model3.fit(X3_train, y3_train)
pred3_svc = svc_model3.predict(X3_test)
print('Book3 SVC Accuracy:', accuracy_score(y3_test, pred3_svc))
print(classification_report(y3_test, pred3_svc))

joblib.dump(svc_model2, "svc_model2.pkl")
joblib.dump(svc_model3, "svc_model3.pkl")

# Save predictions for Book2 
results_book2 = pd.DataFrame({
    'NumFacture': book1.loc[X2_test.index, 'NumFacture'],
    'Book2_Predicted_SVC': pred2_svc,
    'Book2_Actual': y2_test.values
})
results_book2.to_csv('transaction_match_predictions_book2_SVC.csv', index=False)

# Save predictions for Book3 
results_book3 = pd.DataFrame({
    'NumFacture': book1.loc[X3_test.index, 'NumFacture'],
    'Book3_Predicted_SVC': pred3_svc,
    'Book3_Actual': y3_test.values
})
results_book3.to_csv('transaction_match_predictions_book3_SVC.csv', index=False)
