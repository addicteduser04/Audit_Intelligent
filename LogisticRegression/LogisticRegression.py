...
print('Book2 LogisticRegression Accuracy:', accuracy_score(y2_test, pred2_lr))
print(classification_report(y2_test, pred2_lr))
...
pred3_lr = lr_model3.predict(X3_test)
print('Book3 LogisticRegression Accuracy:', accuracy_score(y3_test, pred3_lr))
print(classification_report(y3_test, pred3_lr))

joblib.dump(lr_model2, "lr_model2.pkl")
joblib.dump(lr_model3, "lr_model3.pkl")

# Save predictions for Book2 
results_book2 = pd.DataFrame({
    'NumFacture': book1.loc[X2_test.index, 'NumFacture'],
    'Book2_Predicted_LR': pred2_lr,
    'Book2_Actual': y2_test.values
})
results_book2.to_csv('transaction_match_predictions_book2_LR.csv', index=False)

# Save predictions for Book3 
results_book3 = pd.DataFrame({
    'NumFacture': book1.loc[X3_test.index, 'NumFacture'],
    'Book3_Predicted_LR': pred3_lr,
    'Book3_Actual': y3_test.values
})
results_book3.to_csv('transaction_match_predictions_book3_LR.csv', index=False)
